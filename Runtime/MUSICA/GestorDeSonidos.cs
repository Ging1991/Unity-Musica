...
namespace Ging1991.Musica {

	public class GestorDeSonidos : MonoBehaviour {

		private DireccionRecursos direccion;
		...
		public void Inicializar(DireccionRecursos direccion) {
		...
		private AudioClip LeerAudio(string nombre) {
			AudioClip archivo = Resources.Load<AudioClip>(direccion.Generar(nombre));
			if (archivo == null)
				Debug.LogWarning($"No se encontró el audio: {nombre}");
			return archivo;
		}
